...
const ThesisFormItem: React.FC<ComponentProps> = ({ initThesis }) => {
  const [duration, setDuration] = useState<RangeValue<Moment | undefined> | undefined>(
    initThesis ? initThesis.duration : [undefined, undefined]
  );
  const [lecturerTopicRegister, setLecturerTopicRegister] = useState<
    EventValue<Moment | undefined>
  >(initThesis ? (initThesis.lecturerTopicRegister as Moment) : undefined);
  const [studentTopicRegister, setStudentTopicRegister] = useState<EventValue<Moment | undefined>>(
    initThesis ? (initThesis.studentTopicRegister as Moment) : undefined
  );
  const [progressReport, setProgressReport] = useState<EventValue<Moment | undefined>>(
    initThesis ? (initThesis.progressReport as Moment) : undefined
  );
  const [review, setReview] = useState<EventValue<Moment | undefined>>(
    initThesis ? (initThesis.review as Moment) : undefined
  );
  const [defense, setDefense] = useState<EventValue<Moment | undefined>>(
  ...
  const dateRangeForState = (
    date: Moment,
    startState?: EventValue<Moment | undefined>,
    endState?: EventValue<Moment | undefined>
  ): boolean => {
    if (!duration) {
      return false;
    }
    ...
    return (beginRange || endRange) as boolean;
  };
  ...
  return (
    <Space direction="vertical">
      ...
      <Form.Item
        ...
        <DatePicker
          format={format}
          disabledDate={(current) => dateRangeForState(current, undefined, studentTopicRegister)}
